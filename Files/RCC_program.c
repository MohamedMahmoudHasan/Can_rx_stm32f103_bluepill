...
static uint8 RCC_ERROR = 0  ;  /*THIS VARIABLE SHOULD HOLD A VALUE THAT INDICATE THE TYPE AND LOCATION OF ERROR*/
/***********************************************************************************************************************************************************************************************************************************************************/
void RCC_InitysClock(void){                      /*CHOOSING CLOCK SYSTEM FROM PREVIOUS OPTIONS*/
           ...
           SET_BIT(RCC_CR,RCC_HSE_ON);             /*ENABLE EXTERNAL CLOCK SOURCE BIT*/	
	         ...
	         CLEAR_BIT(RCC_CR , RCC_HSE_BYP);	     /*DISABLE EXTERNAL CLOCK BY PASS*/ 
	        ...
	        while(WAIT_HSE_GET_READY);      /*WAIT TILL HSE CLOCK IS READY*/
	          ...
	          SET_BIT  (RCC_CR   , RCC_HSI_ON);       /*ENABLE INTERNAL CLOCK SOURCE BIT*/
		        ...
		        CLEAR_BIT(RCC_CFGR , RCC_SW0);          /*ADJUST SYSTEM CONTROL SWITCH*/ 
	         	CLEAR_BIT(RCC_CFGR , RCC_SW1);
	         ...
	         RCC_CFGR |=  ((RCC_PLL_MAL_VAL - 2 ) << RCC_PLL_MUL0);         /* SET NEW MUL 4BITS VALUE*/
		    	...
		    	SET_BIT  (RCC_CR, RCC_HSI_ON ); /*ENBLE INTERNAL CLOCK */
		     	CLEAR_BIT(RCC_CFGR , RCC_PLL_SRC);  /*INTERNAL CLOCK DEVIDED BY 2 AS SOURCE OF PLL*/
      ...
      #error "wrong clock configuration input"  /*if you get here then you choose wrong clock choice*/
		  ...
		  CLEAR_BIT(RCC_CFGR , RCC_MCO_2);    /*SET 1BITS ACCORDING TO CLOCK TYPE OTHER 2 IS NEGLIBALE*/
		 		 
#elif RCC_MCO_PIN == SYSTEM_CLOCK
		  CLEAR_BIT(RCC_CFGR,RCC_MCO_0);     /*SET 3BITS ACCORDING TO SYSTEM CLOCK TYPE*/
		...
		CLEAR_BIT(RCC_CFGR,RCC_MCO_0 );  /*SET 3BITS ACCORDING TO EXTERNAL CLOCK TYPE*/
		...
		#error "wrong clock configuration input"
  		...
  		CLEAR_BIT(RCC_CFGR,RCC_ADCPRE0);      /*SET 2BIT ACCORDING TO PRESCALE CHOICE*/
...
#error "wrong clock configuration input"
		  ...
		  CLEAR_BIT(RCC_CFGR,RCC_PPRE2_2);       /*SET 1BITS ACCORDING TO PRESCALE TYPE OTHER 2 IS NEGLIBALE*/
			
#elif RCC_APB2_PRESCALLER == RCC_PRESCALLER_BY_2   /*CLOCK DEVIDED BY 2*/
		  CLEAR_BIT(RCC_CFGR,RCC_PPRE2_0);       /*SET 3BITS ACCORDING TO PRESCALE CHOICE*/
...
#error "wrong clock configuration input"
		  ...
		  CLEAR_BIT(RCC_CFGR , RCC_PPRE1_2);        /*SET 1BITS ACCORDING TO PRESCALE TYPE, OTHER 2 IS NEGLIBALE*/
			                                         
#elif RCC_APB1_PRESCALLER == RCC_PRESCALLER_BY_2    /*CLOCK DEVIDED BY 2*/
		  CLEAR_BIT(RCC_CFGR,RCC_PPRE1_0);        /*SET 3BITS ACCORDING TO PRESCALE CHOICE*/
	...
	#error "wrong clock configuration input"
	  	...
	  	CLEAR_BIT(RCC_CFGR , RCC_HPRE3);          /*SET 1BITS ACCORDING TO PRESCALE TYPE, OTHER 3 IS NEGLIBALE*/
			                                          
#elif RCC_AHP_PRESCALLER == RCC_PRESCALLER_BY_2    /*CLOCK DEVIDED BY 2*/
		  CLEAR_BIT(RCC_CFGR,RCC_HPRE0);          /*SET 4BITS ACCORDING TO PRESCALE CHOICE*/
	...
	if (Copy_u8PerId <= 31){             /*PERIPHERAL NUMBER BIT SHOULD BE LESS THAN 32 */
     ...
     switch(Copy_u8BusId)          /*CHOOSE ONE OF THE BUSES*/
		 { 
     	 ...
     	 case   RCC_APB1_EN : SET_BIT( RCC_APB1ENR , Copy_u8PerId);    break;       /*SET PERIPHERAL BIT LOCATED IN APB1 BUS*/
			 ...
			 default :                                       /*IF YOU GET HERE THEN YOU CHOOSE WRONG BUS NUMBER*/      
              ...
              RCC_ERROR = WRONG_BUS_EN_CLK;        /*WRONG BUS NUMBER */
		 ...
		 else
		 {
              ...
              RCC_ERROR = WRONG_PER_EN_CLK;        /*WRONG PERIPHERAL NUMBER */
	   }
...
void RCC_VoidDisableClock(uint8 Copy_u8BusId ,            /*DISABLE CLOCK OF A CERTAIN PERIPHERAL BIT IN A CERTAIN BUS*/
    ...
    if (Copy_u8PerId <= 31){                   /*PERIPHERAL NUMBER BIT SHOULD BE LESS THAN 32 */
         ...
         switch(Copy_u8BusId)              /*CHOOSE ONE OF THE BUSES*/
		            {
			     case    RCC_AHB_EN :  CLEAR_BIT( RCC_AHBENR , Copy_u8PerId);       break;       /*CLEAR PERIPHERAL BIT LOCATED IN AHB BUS*/
			                                                                                      
     	     case    RCC_APB1_EN : CLEAR_BIT( RCC_APB1ENR , Copy_u8PerId);      break;       /*CLEAR PERIPHERAL BIT LOCATED IN APB1 BUS*/
		                                                                                        
	    	 	 case    RCC_APB2_EN : CLEAR_BIT( RCC_APB2ENR , Copy_u8PerId);      break;       /*CLEAR PERIPHERAL BIT LOCATED IN APB2 BUS*/
			 
			     default :                                   
               						 /*IF YOU GET HERE THEN YOU CHOOSE WRONG BUS NUMBER*/      

                  RCC_ERROR = WRONG_BUS_DIS_CLK;        /*WRONG BUS NUMBER */
		 ...
		 else
		 {
                ...
                RCC_ERROR = WRONG_PER_DIS_CLK;       /*WRONG PERIPHERAL NUMBER */  
	   }
...
void RCC_VoidResetPeriph(uint8 Copy_u8BusId ,              /*RESET A CERTAIN PERIPHERAL IN A CERTAIN BUS*/
	...
	if (Copy_u8PerId <= 31){            /*PERIPHERAL NUMBER BIT SHOULD BE LESS THAN 32 */
     ...
     switch(Copy_u8BusId)          /*CHOOSE ONE OF THE BUSES*/
		 { 
     	 ...
     	 case   RCC_APB1_RES :  SET_BIT( RCC_APB1RSTR , Copy_u8PerId);      break;        /*SET PERIPHERAL BIT LOCATED IN APB1 BUS*/
			 ...
			 default :                                   
              ...
              RCC_ERROR = WRONG_BUS_RES_PER;    /*WRONG BUS NUMBER */  
		 ...
		 {
                                                 /*IF YOU GET HERE THEN YOU CHOOSE WRONG BUS NUMBER*/      
              RCC_ERROR = WRONG_PER_RES_PER;         /*WRONG PERIPHERAL NUMBER */
	   }
